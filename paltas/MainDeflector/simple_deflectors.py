...
class PEMD(MainDeflectorBase):
	...
	def draw_main_deflector(self):
		...
		for model in md_model_list:
			...
			p_names = ProfileListBase._import_class(model,custom_class=None,
					   kwargs_interp=None).param_names
					   #kwargs_interp=None,kwargs_synthesis=None).param_names
			model_kwargs = {}
...
class SPLCORE(MainDeflectorBase):
	"""Class for rendering a main deflector the includes a spherical power law
    and a cored component.
	...
	"""
	# Define the parameters we expect to find for the DG_19 model
	required_parameters = ('sigma0','r_core','gamma','center_x',
		'center_y','z_lens')
